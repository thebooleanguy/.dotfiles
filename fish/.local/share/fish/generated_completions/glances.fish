...
complete -c glances -s V -l version -d 'show programâs version number and exit'
...
complete -c glances -l light -l enable-light -d 'light mode for Curses UI (disable all but top menu)'
...
complete -c glances -s w -l webserver -d 'run Glances in web server mode (bottle lib needed)'
complete -c glances -l cached-time -d 'set the server cache time [default: 1 sec] open-web-browser try to open the W…'
...
complete -c glances -l hide-kernel-threads -d 'hide kernel threads in process list (not available on Windows)'
complete -c glances -s b -l byte -d 'display network rate in byte per second'
...
complete -c glances -l theme-white -d 'optimize display colors for white background'
complete -c glances -l disable-check-update -d 'disable online Glances version ckeck'
